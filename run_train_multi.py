...
def model_pipeline(config, verbose=0): 
    ...
    model, data_loaders, device, criterion, optimizer = make(config)

    # and use them to train the model
    train(model, data_loaders, device, criterion, optimizer, config)
...
def make(config): 
    ...
    return model, data_loaders, device, criterion, optimizer

def train(model, loaders, device, criterion, optimizer, config): 
    ...
    for e, epoch in enumerate(range(config.epochs)):
        ...
        for loader in tqdm(loaders, desc='loader'):
            for data in tqdm(loader, desc='batch'):
                # get the images
                images = data['img']
